...
pub async fn build_dashboard(state: &WebState, caller: &User) -> DashboardModel {
    ...
    let (gauges, load, disk_mounts, monitoring_error, last_updated) = match state
        .monitoring
        .snapshot_now()
        .await
    {
...
fn quick_actions_for(is_owner: bool) -> Vec<QuickAction> {
    let mut actions = vec![
        QuickAction {
            label: "Manage sites",
            href: "/sites",
...
fn gauges_from_snapshot(snapshot: &SystemSnapshot) -> (Vec<GaugeWidget>, f64) {
...
fn status_for_percent(pct: f64) -> ResourceStatus {
    if !pct.is_finite() {
...
mod tests {
    ...
    fn gauges_render_all_values_with_status() {
        ...
        assert!(out.contains("hx-trigger=\"every 30s\""), "auto-refresh trigger");
    ...
    fn user_quick_actions_omit_owner_only() {
        ...
        assert!(actions.iter().all(|a| a.href != "/backups" && a.href != "/security"),
    ...
    fn cards_render_counts_and_links() {
        ...
        for (href, label, count) in [
            ...
            assert!(out.contains(&format!("{count}")), "count {count} for {label}");
        }
    }
