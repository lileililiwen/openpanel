//! Deployment-adapter domain types: the data shapes the
//! bounded context owns (manifest, plan, evidence, action,
//! state, idempotency, health, secret reference) and the
//! pure redaction helper. The validate / decide / record
//! logic lives in `logic.rs`; the unit + property tests
//! live in `tests.rs`. This file is I/O-free.
