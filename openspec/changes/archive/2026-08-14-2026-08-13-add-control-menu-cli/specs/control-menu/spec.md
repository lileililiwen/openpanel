...
Introduces a Baota-style numbered-menu CLI for OpenPanel
operators. The menu is delivered as a sibling npm package
(`packages/openpanel-menu/`, also reachable as
`openpanel-menu`); it ships the canonical `opctl` binary and
the `scripts/install.sh` curl|bash installer that bootstraps
Node.js 20 via the system package manager before running
`npm install -g`. The menu wraps `openpanel-cli` (when a
command exists) or talks to systemd + `/etc/openpanel/openpanel.toml`
directly (when one does not), and never manages nginx /
MySQL / Redis / Docker or any service outside OpenPanel.
