...
Adds a server-rendered webmail client running under the panel
session, so that an end user can read and send mail without
configuring a desktop client. Authentication is delegated
through a short-lived (≤15 minute) mailbox password that the
panel mints and rotates; the user's actual mailbox password
is never revealed to or stored in the browser.
