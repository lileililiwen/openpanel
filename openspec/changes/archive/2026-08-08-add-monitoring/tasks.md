...
- [x] 2.1 Mock-based test in `openpanel-app/src/monitoring/service.rs`:
...
- [x] 2.2 Mock-based test: `latest()` returns the newest snapshot;
      when the repo is empty it returns `None`.
- [x] 2.3 Mock-based test: `history(kind, since)` delegates to the
      repo and preserves ascending order.
- [x] 2.4 SQLite repo test (`openpanel-app/src/monitoring/repo.rs`,
      `#[cfg(test)]`, real SQLite via `TestDb`):
      `insert` → `latest` → `history` round-trip, plus `prune(before)`
      removes old rows and keeps new ones.
- [x] 2.5 SQLite repo test: `history` filters by kind and is ordered
      ascending by timestamp; `prune` deletes only rows older than
      `before`.

## 3. Testing — Collector + Background Task

- [x] 3.1 Unit test in `openpanel-app/src/monitoring/collector.rs`:
...
- [x] 3.3 Mock-based test: a failing collector tick logs the error and
      the task continues (next tick still collects).
- [x] 3.4 Unit test: `retention_days = 0` disables pruning (no
      `prune` call on tick).

## 4. Testing — Alert Evaluation

- [x] 4.1 Unit test in `openpanel-app/src/monitoring/alerts.rs`:
      `AlertRule::evaluate(value)` returns `Some(alert)` when
      `value > threshold`, `None` otherwise.
- [x] 4.2 Unit test: hysteresis — a sequence of ticks
      `[95, 95, 95, 50, 95]` with `cpu_percent = 90` produces exactly
      two `AlertFired` events (first crossing, and re-crossing after
      dropping below).
- [x] 4.3 Unit test: an absent threshold (unset) never fires.
- [x] 4.4 Mock-based test: when an alert fires, `AuditService.record`
      is called with `AuditAction::AlertFired` and target = metric
      kind.

## 5. Testing — HTTP Routes

- [x] 5.1 Integration test (`tests/integration/monitoring.rs`):
      with a `TestServer`, `GET /api/v1/monitoring/overview` returns
      `200` and a JSON body containing `timestamp`, `cpu`, `memory`.
- [x] 5.2 Integration test: `GET /api/v1/monitoring/history?metric=Cpu`
      returns `200` and an array (may be empty on a fresh DB).
- [x] 5.3 Integration test: `metric=Bogus` returns `400` with error
      code `invalid_metric`.
- [x] 5.4 Integration test: no session → `401` for `/overview`,
      `/history`, and `/alerts`.

## 6. Testing — CLI

- [x] 6.1 CLI E2E (`tests/cli/monitoring.rs`):
      `openpanel monitoring overview` exits 0 and prints cpu % /
      memory % / load.
- [x] 6.2 CLI E2E: `openpanel monitoring history --metric Disk`
      exits 0 (may print "no samples" on a fresh DB) and exits
      non-zero with a clear message for an unknown metric.

## 7. Domain Layer

- [x] 7.1 Create `crates/openpanel-domain/src/monitoring/mod.rs`
      re-exporting `MetricKind`, `Unit`, `MetricSample`,
      `SystemSnapshot`, `SnapshotRepository`, `MonitoringError`,
      `Alert`, `AlertRule`.
- [x] 7.2 Create `crates/openpanel-domain/src/monitoring/metric.rs`
      with `MetricKind`, `Unit`, `MetricSample`, `clamp_to_percent`.
- [x] 7.3 Create `crates/openpanel-domain/src/monitoring/snapshot.rs`
      with `SystemSnapshot` (`new`, `samples`, `cpu`, `memory`,
      `disk`, `network`, `load`, `timestamp`).
- [x] 7.4 Create `crates/openpanel-domain/src/monitoring/repository.rs`
      with the `SnapshotRepository` trait.
- [x] 7.5 Create `crates/openpanel-domain/src/monitoring/error.rs`
      with `MonitoringError` variants: `NotFound`, `Repo`,
      `InvalidKind`, `InvalidValue`, `Io`, `TimestampInFuture`,
      `EmptySnapshot`.
- [x] 7.6 Create `crates/openpanel-domain/src/monitoring/alert.rs`
      with `Alert` and `AlertRule` value objects.
- [x] 7.7 Re-export `monitoring::*` from
      `crates/openpanel-domain/src/lib.rs`.

## 8. Application Layer — Service + Repository

- [x] 8.1 Create `crates/openpanel-app/src/monitoring/mod.rs`.
- [x] 8.2 Create `crates/openpanel-app/src/monitoring/repo.rs` with
      `SqliteSnapshotRepository`.
- [x] 8.3 Create `crates/openpanel-app/src/monitoring/service.rs` with
      `MonitoringService::new(...)` and methods
      `record / latest / history / prune / snapshot_now /
      evaluate_alerts`.
- [x] 8.4 Re-export `MonitoringService` from
      `crates/openpanel-app/src/lib.rs`.

## 9. Application Layer — Collector + Task

- [x] 9.1 Create `crates/openpanel-app/src/monitoring/collector.rs`
      with a `Collector` trait + `SystemCollector` (`sysinfo`-backed).
- [x] 9.2 Create `crates/openpanel-app/src/monitoring/alerts.rs` with
      the alert evaluator + hysteresis state.
- [x] 9.3 Create `crates/openpanel-app/src/monitoring/task.rs` with
...
- [x] 10.2 Add `pub const MONITORING_V001` to
      `crates/openpanel-app/src/migrations/mod.rs`.
- [x] 10.3 Create `crates/openpanel-app/src/monitoring/module.rs`
      with `MonitoringModule::new(ctx, collector)` registering the
      service, migration, background task, and routes.
- [x] 10.4 Re-export `MonitoringModule` from
      `crates/openpanel-app/src/lib.rs`.

## 11. HTTP Routes

- [x] 11.1 Create `crates/openpanel-api/src/dto/monitoring.rs`.
- [x] 11.2 Create `crates/openpanel-api/src/routes/monitoring.rs` with
      `pub fn router(svc: Arc<MonitoringService>) -> Router` exposing
      `/overview`, `/history`, `/alerts`.
- [x] 11.3 Update `crates/openpanel-api/src/router.rs` to take
      `monitoring: Arc<MonitoringService>` and nest `/monitoring`.
- [x] 11.4 Add `AlertFired` to `AuditAction`.

## 12. CLI Subcommands

- [x] 12.1 Add `MonitoringCommand` enum to
      `crates/openpanel-cli/src/commands.rs`.
- [x] 12.2 Implement handlers in `crates/openpanel-cli/src/handlers.rs`
      (`overview`, `history`).
- [x] 12.3 Wire the subcommand in `main.rs` under `Command::Monitoring`.

## 13. Composition Root Wiring

- [x] 13.1 Wire `MonitoringModule` in
      `openpanel-test-support/src/server.rs` (sandboxed DB, real
      `SystemCollector` so integration tests see real values).
- [x] 13.2 Wire `MonitoringModule` in `openpanel-cli/src/handlers.rs`
      (`serve`).
- [x] 13.3 Add the `[monitoring]` section (interval_secs,
      retention_days, alert.*) to the config defaults + schema.

## 14. Documentation

- [x] 14.1 Update root `README.md`: add a "Monitoring" section and a
      row in the capabilities table.
- [x] 14.2 Add `crates/openpanel-app/src/monitoring/README.md`.
- [x] 14.3 Update `tests/README.md` with the new
      `tests/integration/monitoring.rs` + `tests/cli/monitoring.rs`
      categories.
- [x] 14.4 Update `Agents.md` References if new artifacts are added.

## 15. Validation

- [x] 15.1 `cargo test --workspace` passes — all existing tests plus
      the new monitoring tests.
- [x] 15.2 `cargo clippy --workspace --all-targets -- -D warnings`
      passes.
- [x] 15.3 `cargo fmt --all -- --check` passes.
- [x] 15.4 `make check` exits 0.
- [x] 15.5 Manual smoke: boot `TestServer` / `openpanel serve`, curl
      `/api/v1/monitoring/overview`, confirm real host values.
- [x] 15.6 Commit + archive via OpenSpec.
