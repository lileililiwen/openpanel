...
Implements runtime locale negotiation, catalog lookup,
plural-form selection, and locale-aware formatting so that
the web shell and CLI can speak a User's language. Hooks the
contract established by
`refine-web-ui-with-audit-accessibility-theming` (T(key) calls)
and the lint introduced by
`refine-quality-with-i18n-and-theme-policy`. Bundles a default
English catalog and supports signed community translations.
