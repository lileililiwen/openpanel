...
- [ ] 1.1 Unit test in `openpanel-domain/src/ssl/certificate.rs`:
...
- [ ] 1.2 Property test in `openpanel-domain/src/ssl/certificate.rs`
      asserting the status classifier:
      `now > valid_to → Expired`; `valid_to - now < 30 days →
      Expiring`; otherwise `Active`.
- [ ] 1.3 Property test for `CertificateRepository::insert`
      ...
      (no SQLite).
- [ ] 1.4 Unit test for `SslError::KeyMismatch` construction +
      `Display`; ensure no private-key material is in the
      formatted error.

## 2. Testing — Application Layer (Service)

- [ ] 2.1 Mock-based test in `openpanel-app/src/ssl/service.rs`:
...
- [ ] 3.1 Unit test in `openpanel-app/src/ssl/renewal.rs`:
...
- [ ] 3.2 Unit test: renewal failure records `last_error` and
      leaves the existing cert in place (does NOT delete the row).

## 4. Testing — ACME HTTP-01 Challenge Server

- [ ] 4.1 Unit test: `AcmeHttpServer::register(domain, token,
...
- [ ] 4.2 Unit test: an unknown token returns 404.
- [ ] 4.3 Integration test (`tests/integration/ssl.rs`): with a
...
- [ ] 5.1 Unit test: `NginxConfigGenerator::render_with_ssl(site,
...
- [ ] 5.2 Unit test: `render_with_force_https(site, true)` on the
      port-80 vhost emits `return 301 https://$host$request_uri;`
      and NO other `location` blocks.
- [ ] 5.3 Unit test: `render_with_force_https(site, false)` emits
      the HTTP vhost unchanged (except the acme-challenge proxy
      block).
- [ ] 5.4 Integration test (`tests/integration/ssl.rs`): boot
...
- [ ] 6.1 Create `crates/openpanel-domain/src/ssl/mod.rs`
      re-exporting `Certificate`, `CertificateSource`,
      `CertificateStatus`, `CertificateRepository`, `SslError`,
      `KeyType`.
- [ ] 6.2 Create `crates/openpanel-domain/src/ssl/certificate.rs`
      with the `Certificate` aggregate (`new`, `status`,
      `is_expiring`, `is_expired`, `decrypt_key`).
- [ ] 6.3 Create `crates/openpanel-domain/src/ssl/source.rs` with
      `CertificateSource` (`Acme`, `Manual`, `SelfSigned`) and
      `CertificateStatus` (`Active`, `Expiring`, `Expired`,
      `Revoked`).
- [ ] 6.4 Create `crates/openpanel-domain/src/ssl/error.rs` with
      `SslError` variants: `NotFound`, `Repo`, `AcmeChallenge`,
      `Acme(String)`, `KeyMismatch`, `Expired`, `InvalidPem`,
      `Io`, `Encryption`, `Decryption`.
- [ ] 6.5 Create `crates/openpanel-domain/src/ssl/repository.rs`
      with the `CertificateRepository` trait.
- [ ] 6.6 Re-export `ssl::*` from `crates/openpanel-domain/src/lib.rs`.

## 7. Application Layer — Service + Repository

- [ ] 7.1 Create `crates/openpanel-app/src/ssl/mod.rs`.
- [ ] 7.2 Create `crates/openpanel-app/src/ssl/repo.rs` with
      `SqliteCertificateRepository`.
- [ ] 7.3 Create `crates/openpanel-app/src/ssl/service.rs` with
...
- [ ] 7.4 Re-export `SslService` from `crates/openpanel-app/src/lib.rs`.

## 8. Application Layer — PEM Crypto + Parsing

- [ ] 8.1 Create `crates/openpanel-app/src/ssl/crypto.rs` with
...
- [ ] 8.2 Create `crates/openpanel-app/src/ssl/parser.rs` with
...
- [ ] 9.1 Create `crates/openpanel-app/src/ssl/acme.rs` with
      ...
      -> Result<IssuedCert>`.
- [ ] 9.2 `AcmeEndpoint::Staging` and `AcmeEndpoint::Production`
      with the correct URLs.

## 10. Application Layer — Self-Signed Generator

- [ ] 10.1 Create `crates/openpanel-app/src/ssl/self_signed.rs`
...
- [ ] 11.1 Create `crates/openpanel-app/src/ssl/challenge_server.rs`
      with `AcmeHttpServer::bind(port: u16) -> Result<Self>` and
      `register(domain, token, key_authorization)` /
      `serve(router) -> Router`.
- [ ] 11.2 The server binds to `127.0.0.1:<port>` only (not
      `0.0.0.0`).

## 12. Application Layer — Renewal Background Task

- [ ] 12.1 Create `crates/openpanel-app/src/ssl/renewal.rs` with
...
- [ ] 12.2 Register the task via `SslModule::background_tasks(ctx)`.
...
- [ ] 13.1 Create
      ...
      status, expires_at).
- [ ] 13.2 Add `pub const SSL_V001` to `migrations/mod.rs`.
- [ ] 13.3 Create `crates/openpanel-app/src/ssl/module.rs` with
...
- [ ] 13.4 Wire `SslModule` into the composition root:
      ...
      - `openpanel-cli/src/handlers.rs` instantiates the module
        in `serve`.
- [ ] 13.5 Re-export `SslModule` from `crates/openpanel-app/src/lib.rs`.

## 14. HTTP Routes

- [ ] 14.1 Create `crates/openpanel-api/src/dto/ssl.rs` with the
      request/response DTOs (`IssueAcmeRequest`,
      `ManualUploadRequest`, `SelfSignedRequest`,
      `ForceHttpsRequest`, `CertificateMetadataDto`).
- [ ] 14.2 Create `crates/openpanel-api/src/routes/ssl.rs` with
      `pub fn router(svc: Arc<SslService>) -> Router` implementing
      the 7 endpoints listed in the spec.
- [ ] 14.3 Update `crates/openpanel-api/src/router.rs` to take
      `ssl: Arc<SslService>` and nest `/ssl`.
- [ ] 14.4 Add `SslRevoked` to `AuditAction` in
      `openpanel-core/src/audit.rs`.

## 15. CLI Subcommands

- [ ] 15.1 Add `SslCommand` enum to
      `crates/openpanel-cli/src/commands.rs`.
- [ ] 15.2 Implement handlers in
      `crates/openpanel-cli/src/handlers.rs` that delegate to
      `SslService` (same code path as the HTTP API).
- [ ] 15.3 Wire the subcommand in `commands.rs`'s top-level
      `Command::Ssl(SslCommand)` arm.

## 16. nginx Render Extension

- [ ] 16.1 Extend `NginxConfigGenerator::render` to take an
      `Option<&Certificate>` parameter; emit the `:443` vhost
      when present.
- [ ] 16.2 Extend `render` to emit the acme-challenge proxy
      block on the port-80 vhost.
- [ ] 16.3 Extend `render` to emit the force-https redirect when
      `force_https = true` AND a cert is active.
- [ ] 16.4 Update `SitesService::create_site`,
      ...
      it to the render.
- [ ] 16.5 Update `SitesModule::with_paths` to take a
      `CertificateRepository` (or pull it from `ctx`) and inject
      it into `SitesService`.

## 17. Documentation

- [ ] 17.1 Update root `README.md` with an "SSL" section
      describing ACME HTTP-01, manual upload, self-signed,
      auto-renewal, force-HTTPS, and the staging-default safety
      net.
- [ ] 17.2 Update `Agents.md` § Security with the new private-key
      confidentiality rules.
- [ ] 17.3 Add `crates/openpanel-app/src/ssl/README.md` covering
      the API surface, the renewal policy, and the storage
      envelope.
- [ ] 17.4 Update `tests/README.md` with the new
      `tests/integration/ssl.rs` test category.

## 18. Validation

- [ ] 18.1 `cargo test --workspace` passes — all existing tests
      plus the new ssl tests.
- [ ] 18.2 `cargo clippy --workspace --all-targets -- -D warnings`
      passes.
- [ ] 18.3 `cargo fmt --all -- --check` passes.
- [ ] 18.4 `make check` exits 0.
- [ ] 18.5 Manual smoke: `make test` against the staging endpoint
      (network-dependent; documented in `tests/README.md`).
- [ ] 18.6 Commit + archive via OpenSpec.
