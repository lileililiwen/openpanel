...
- **THEN** `make check` fails with `scan: literal rgb color outside tokens.css` and the offending file and line number. The fix is to declare `--op-color-accent-rgb: 79, 140, 255;` in `tokens.css` and reference it as `rgba(var(--op-color-accent-rgb), 0.1)`.
